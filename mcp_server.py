...
def main():
    try:
        ...
        logger.info("  🩺 get_cluster_health: Full cluster health snapshot (slots, TMs, jobs, assessment)")
        logger.info("-" * 60)
        logger.info("  📋 list_jobs: List all Flink jobs with status")
        logger.info("  🔍 get_job_details: Comprehensive job details by ID")
        logger.info("  🩺 diagnose_job: Unified health report (exceptions + metrics + checkpoints + details)")
        logger.info("  📜 get_job_history: Run history for a job by name (success rate, avg duration, failures)")
        logger.info("  📊 compare_checkpoints: Checkpoint trend analysis (duration/size trends, outliers, assessment)")
        ...
        logger.info("-" * 60)
        logger.info("  🔬 get_vertex_details: Full per-subtask breakdown + accumulators for a vertex")
        logger.info("  🔥 get_vertex_flamegraph: CPU flame graph data for a vertex (Flink 1.17+)")
        logger.info("  🔍 find_bottleneck: Identify the bottleneck operator by backpressure ranking")
        logger.info("-" * 60)
        logger.info("  💻 list_taskmanagers: List TaskManagers with resources")
        logger.info("  🖥️  get_taskmanager_details: Deep-dive into a specific TaskManager")
        logger.info("  📊 get_taskmanager_metrics: Query or list TaskManager metrics")
        logger.info("  🧵 get_taskmanager_thread_dump: JVM thread dump grouped by state")
        logger.info("  🩺 diagnose_taskmanager: Full TM health report (CPU, heap, GC, threads, assessment)")
        logger.info("-" * 60)
